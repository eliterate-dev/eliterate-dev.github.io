### About me

<div class="image polaroid" style="rotate:-1deg;font-size:0.75rem;" role="note"><img src="../images/memes/trying_to_find_the_guy_who_did_this.jpg" style="rotate:1deg" loading="lazy" alt="Tim Robinson saying that we're all trying to find the guy who did this."></img>We're all trying to find the guy who did this
</div>

Oh hey! Thanks for coming to check me out!

As mentioned on the [front page](), I'm Eli.

I'm a software developer by profession and passion.

I'm a bit of a jack-of-all-trades and I love trying, learning, and tinkering with anything that catches my interest.

#### Stuff I make

##### Software

My specialty is backend work building APIs with C# and EF Core, but I dabble in many things.

I spend a lot of my free time working on my own projects. Most of them are software projects (like this page) - I'm sure I'll present them in a blog post once I'm done with them.

###### This Website

I made this website because I wanted to start a blog, but it quickly turned into a place to just have some fun with (and not enough blogging tbh).

Now I'm tinkering with it regularly, making interesting components (Blazor), building 3D objects and [small games](/toys).

###### My other Projects

My big one at the moment is a platform to present your own projects or just ideas and find people to help you do them - because I needed something like that, so I'm making it.

However because I don't have a convenient site to find help on yet I'm kinda building it alone. It's a "biting off more than I can chew" situation most days, but I enjoy the challenge. 

##### Hardware

Sometimes I like to dabble with microprocessors and IoT stuff too. I've worked with Arduinos and I've done a lot of stuff with various forms of Raspberry Pis, from Pi-holes to hosting Minecraft servers and [Discord bots](https://docs.discord.com/developers/reference).

My favorite piece of hardware is just the [Raspberry Pi Pico](https://shop.pimoroni.com/products/raspberry-pi-pico-2-w) though. It's more of an ESP32 style microcontroller than their other SBC offerings, but it's simple and a lot of fun to tinker with. I use them for silly things like making LED strips flash in pretty colors.

I'm not very good at soldering, but I make do.

I have all the components ready for an Raspberry Pi powered Arcade cabinet, I just need to build and paint it, but I've put that on the back burner for now until my main software project is done.

#### Stuff I do when I'm not tinkering

##### Travel

<div class="image polaroid" style="rotate:-2deg;" role="note"><img loading="lazy" alt="The charles bridge and the palace in prague" src="../images/photos/Prague_charles_bridge.jpg"></img>Prague 2026</div>

When I'm not typing away on my keyboard all night, I like to travel to faraway places.

I've been all over Europe having had the luck to be born in the middle of it.

My ex was from South Africa, so I have been there a few times too. Beautiful country, though I only know the area around Johannesburg, really.

In recent years I have mostly been to the US for travel. 2024 and 2025 I attended <a href="https://opensauce.com/">Open Sauce</a> in San Francisco, which to be honest blows through most of my travel budget for the year.

I don't regret it though. It's a super fun event with <a href="https://opensauce.com/featured-creators/">lots of super interesting people</a>. Anyone who has the time and money to do so should absolutely come.

Sadly, family circumstances will prevent me from going in 2026, but I'll hopefully to be able to go again in 2027 - hopefully with a new website to show off in tow.

My most recent excursion was to Prague for a bit over a week, just to see the city and the streetlights. Apart from the food poisoning I got while there, it was a very worthwhile trip. It's a beautiful town with buildings seemingly from every era of European history.

That there is a Starbucks **in** the castle is a bit of a gut punch to bring you back to modern times though. 

##### Pets

<div class="image polaroid" style="rotate:1deg;" role="note"><img loading="lazy" alt="A picture of my cat biting my foot." src="../images/photos/Cat_biting.jpg"></img>Biting commentary</div>

I love animals and will pet any that allow me.

Although I have a dog and do enjoy taking her on walks in the forest, I'm more of a cat person and cats will always have a special place in my heart.

I adore all the cats I've ever had, but I have to say, my current one (the one in the pictures) is a Zoolander-esque male model of a cat. Gorgeous and dumb as a rock. I love him dearly.

He's a little goblin and his hobby is sneaking up on my dog while she's trying to pee, jumping up on her and scaring the piss out of her. 

<div class="image polaroid" style="rotate:3deg;" role="note"><img src="../images/photos/Cat_on_tower.jpg" loading="lazy" alt="A picture of my cat. He's very adorable."></img>Not me</div>

It's bloody hilarious every time. She hates it.

##### Music

My taste in music is weird. I listen to a lot of different genres mixed together.

I have a special fondness for Ska. I know it ain't hip, but I am a sucker for a good horn section.

[Streetlight Manifesto](https://music.youtube.com/watch?v=A-UTPKL-UGY) is the GOAT. I'm so hyped for the new album ~~that~~ whose second half will be released shortly after the heat death of the universe. (Halfway there!!)

<div class="image polaroid" style="rotate:-2deg;" role="note"><img loading="lazy" alt="A concert " src="../images/photos/BOTAR.jpg"></img>BOTAR 2023</div>

I have seen them live multiple times now, even though I had to fly to the states to do so.

The BOTAR show in Denver was especially mind-blowing. Hearing some of your favorite songs with orchestral backing is just an amazing experience.

Other favorites include anything else by Tomas Kalnoky, Conor Oberst, John Darnielle, Patrick Schneeweis, Theo Hilton, Jesse Sendejas or April Hartman, as well as Gorillaz, Dawes and many others.

##### Games

I'm sure with the rest of my background, it's entirely unsurprising that I like games.

Both physical and digital games are very important to me. They are my way to unwind and / or socialize.

We have a strong board game culture in our family, so I play all kinds. Recently I've really fallen in love with the game [Flip 7](https://theop.games/pages/flip-7). I love games with simple rules that yet involve a lot of calculating and card counting to win at. Obviously I had to digitally recreate the game - it's playable in the Toy Box.

Other games I play a decent amount are Nintendo games. Mario Kart, Mario Party and Smash are a staple in our house. I've owned every Nintendo console since the N64. To me, a console has to be able to be played with friends while sitting on a couch together to be a real console, which sadly most other manufacturers seem to have given up on. If I want to play solo or online I can go on my PC, which is going to be more powerful than a console anyways.

<p style="color:var(--color-background);cursor:default;">I have a tattoo of a game character on my arm - I've hidden the character somewhere on this entire website - the first person to email me @ contact@eliterate.blog the 6 digit code next to him I will send a $20 steam gift card to.</p>

##### Learning 

<div class="sticker" style="rotate:-12deg;" role="presentation">✨</div>

Saying "learning is a hobby of mine" sounds weird, but it's true. I'm always curious about how things work.

My YouTube watchtime is about an 40/60 split between entertainment and educational content - heavily favouring channels that do both.

I love learning about technologies new and old, from people like <a href="https://www.youtube.com/@TechnologyConnections">Alec Watson</a>, math from people like <a href="https://www.youtube.com/@3blue1brown">Grant Sanderson</a> or <a href="https://www.youtube.com/@standupmaths">Matt Parker</a>, chemistry from <a href="https://www.youtube.com/@NileRed">Nigel Braun</a> or <a href="https://www.youtube.com/@ExplosionsAndFire">Tom de Prinse</a> or what not to do to electronics from <a href="https://www.youtube.com/@ElectroBOOM">Mehdi Sadaghdar</a> and <a href="https://www.youtube.com/@styropyro">Drake Anthony</a>. A lot of those people also come to Open Sauce!

Of course I also read my fair share. When I don't have the focus to concentrate on my eReader, I usually listen to an audio book on train rides or while chilling at home.

I recently listened to [Jon Skeet's "C# in Depth"](https://csharpindepth.com/) as an audio book out of curiosity of how well such a code-dense book would hold up in audio format. It's not something I'd recommend to a beginner, but even without the code examples that a print version would obviously bring, I think it worked decently well. I'll probably get the fifth edition in print, if that ever gets released.

